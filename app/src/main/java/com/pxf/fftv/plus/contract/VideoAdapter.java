...
class VideoAdapter extends RecyclerView.Adapter<VideoAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        GlideApp.with(context).load(videoList.get(position).getImageUrl()).skipMemoryCache(true)
                .diskCacheStrategy(DiskCacheStrategy.RESOURCE).into(holder.video_list_item_iv_image);
        holder.video_list_item_tv_title.setText(videoList.get(position).getTitle());

        // Display remarks badge if available
        String remarks = videoList.get(position).getRemarks();
        if (remarks != null && !remarks.trim().isEmpty()) {
            holder.video_list_item_tv_remarks.setText(remarks);
            holder.video_list_item_tv_remarks.setVisibility(View.VISIBLE);
        } else {
            holder.video_list_item_tv_remarks.setVisibility(View.GONE);
        }
        ...
        // Trigger load more when approaching the end of the list (within last 2 rows)
        // This ensures smooth continuous pagination
        int triggerPosition = videoList.size() - (VIDEO_LIST_COLUMN * 2);
        if (triggerPosition < 0) {
            triggerPosition = 0;
        }

        if (position >= triggerPosition && !isLoadingMore) {
            if (onStartLoadMoreListener != null) {
                isLoadingMore = true;
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        TextView video_list_item_tv_remarks;
        ...
        private View.OnKeyListener onKeyListener = new View.OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                // Remove manual load more logic (handled by prefetching)
                // Remove debounce logic to improve responsiveness
                return false;
            }
        };
        ...
        ViewHolder(@NonNull View itemView) {
            super(itemView);
            video_list_item_root = itemView.findViewById(R.id.video_list_item_root);
            video_list_item_iv_image = itemView.findViewById(R.id.video_list_item_iv_image);
            video_list_item_tv_title = itemView.findViewById(R.id.video_list_item_tv_title);
            video_list_item_iv_icon = itemView.findViewById(R.id.video_list_item_iv_icon);
            video_list_item_tv_remarks = itemView.findViewById(R.id.video_list_item_tv_remarks);
