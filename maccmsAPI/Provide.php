...
class Provide extends Base
{
    ...
    public function vod()
    {
        ...
        if(empty($html) || $cache_time==0) {
            ...
            if (!empty($this->_param['lvl'])) {
                if (empty($GLOBALS['config']['api']['vod']['typefilter']) || strpos($GLOBALS['config']['api']['vod']['typefilter'], $this->_param['lvl']) !== false) {
                    $where['vod_level'] = $this->_param['lvl'];
                }
            }
            if (!empty($this->_param['t1'])) {
                if (empty($GLOBALS['config']['api']['vod']['typefilter']) || strpos($GLOBALS['config']['api']['vod']['typefilter'], $this->_param['t1']) !== false) {
                    $where['type_id_1'] = $this->_param['t1'];
                }
            }
		...
		if (!preg_match($match, $str)){
			...
			foreach ($chars as $char) {
				$queryStr = $queryStr . $char . '%';
			}
			$searchMode = 'vod_en';
			$newStr = strtolower($queryStr);
		}else{
			$searchMode = 'vod_name';
			$newStr = '%' . $str . '%';
		}
		$where[$searchMode] = ['like', $newStr];
            }
